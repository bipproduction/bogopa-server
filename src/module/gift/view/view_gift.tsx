...
import { funGetListGift } from '..';

export default function ViewGift({ data }: { data: any }) {
  ...
  async function onLoad(p: any) {
    const load = await funGetListGift({ search: isSearch, page: isChoosePage })
    setChoosePage(p)
    setData(load.data)
    setNPage(load.nPage)
  }
  ...
  async function onSearch(s: any) {
    const load = await funGetListGift({ search: s, page: 1 })
    setSearch(s)
    setChoosePage(1)
    setData(load.data)
    setNPage(load.nPage)
